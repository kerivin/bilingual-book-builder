...
- **Quick test on a real book:** a full auto-build aligns every chapter and takes minutes (~8-10 min for some books). To verify a real book fast, build only a few pairs with manual mapping: `bbb -s book.epub -t book_tr.epub -sl en -tl ru -m -o out`. In the interactive prompt, `[ENTER]` matches the shown pair, `T`/`S` skip a chapter, `F` skips the rest and finishes, `y` confirms. Unmatched chapters are dropped unless `--keep-source-chapters` / `--keep-target-chapters`. Keep such a test under ~2 minutes of alignment.
