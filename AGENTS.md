...
- The extractor's chapter dicts carry only `content_html`, never a precomputed plain-text key — the mapper parses `content_html` directly to build auto-match signatures; if signatures ever fall back to title-only, auto-match collapses to a handful of pairs. Don't "simplify" the signature lookup.
...
- Import-order constraint in `BBB.run()`: `bbb.aligner` must be imported **before** sentence-transformers puts `transformers` into `sys.modules` (i.e. keep the Aligner import above `_create_sentence_transformer()`). `bbb.aligner → bbb.splitter → wtpsplit → skops`, and skops' import-time type scan probes every loaded module; if the lazy-loading `transformers` package is already present, that scan pulls a transformers submodule that hard-requires `torchvision` (not installed) and crashes with `ModuleNotFoundError`. Importing them in the other order works fine — the trap only triggers on production's sequence (LaBSE loads during chapter matching, before alignment imports happen), which is also why the test suite can't catch it: tests import `bbb.aligner` directly.
