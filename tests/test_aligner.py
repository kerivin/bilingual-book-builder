...
def test_perfect_one_to_one():
    ...
    with patch.object(aligner.splitter, 'run', side_effect=lambda text, lang:
        [["Hello.", "World."]] if "Hello" in text else [["Hallo.", "Welt."]]), \
...
def test_sentences_missing_in_target():
    ...
    with patch.object(aligner.splitter, 'run', side_effect=lambda text, lang:
        [["A.", "B.", "C."]] if text.startswith("A.") else [["A.", "C."]]), \
         patch('bbb.aligner.Bertalign') as mock_bert:
        # Bertalign returns pairs (0,0), (2,1) – B missing
        mock_bert.return_value.result = [([0], [0]), ([2], [1])]
        mock_bert.return_value.align_sents = MagicMock()
        result = aligner._align_pair("A. B. C.", "A. C.")
        # Should have 3 segments: matched A, unmatched B (source only), matched C
        para = result[0]
        assert len(para) == 3
        assert para[0] == {'source': 'A.', 'target': 'A.', 'source_footnote_occurrences': [], 'target_footnote_occurrences': []}
...
def test_sentences_missing_in_source():
    ...
    with patch.object(aligner.splitter, 'run', side_effect=lambda text, lang:
        [["A.", "C."]] if len(text.split())==2 else [["A.", "B.", "C."]]), \
         patch('bbb.aligner.Bertalign') as mock_bert:
...
def test_empty_source_text():
    ...
    # _align_pair returns early with single segment containing full texts
    result = aligner._align_pair("", "Text")
    assert result == [[{'source': '', 'target': 'Text'}]]

def test_empty_target_text():
    aligner = make_aligner(make_chapters(["Text"]), make_chapters([""]))
    result = aligner._align_pair("Text", "")
    assert result == [[{'source': 'Text', 'target': ''}]]