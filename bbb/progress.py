...
def _start(phase: str, total: int, desc: str = None):
    global _current_phase, _current_total, _current_n
    if _callback is None:
        return
    _current_phase = phase
    _current_total = total
    _current_n = 0
    _callback(phase, 0, _current_total, desc or phase)
...
def update(phase: str, step: int = 1, message: str = None):
    ...
    if _callback is None or phase != _current_phase:
        return
    _current_n += step
    _callback(_current_phase, _current_n, _current_total, message)
...
def _finish(phase: str):
    ...
    if _callback is None or phase != _current_phase:
        return
    if _current_phase:
        _callback(_current_phase, _current_total, _current_total, "Done")
    _current_phase = None
    _current_total = 0
    _current_n = 0